...
class Transformer(nn.Module):
    ...
    def __init__(
            ...
            n_encoder_layers=6, n_decoder_layers=6, n_common_layers=0, 
            n_layers=6, dropout=0.1, n_position=200, swish_activation=False, share_qk=False,
...
class PTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.position_enc = PositionalEncoding(d_word_vec, n_position=n_position)
        self.src_embedding_norm = nn.LayerNorm(d_word_vec, eps=1e-6)
        self.encoder_layers = nn.ModuleList([
            EncoderLayer(d_model, d_inner, n_head, d_k, d_v, dropout=dropout, 
                share_qk=share_qk, swish_activation=swish_activation) for _ in range(n_encoder_layers)])
        ...
        self.trg_embedding_norm = nn.LayerNorm(d_word_vec, eps=1e-6)
